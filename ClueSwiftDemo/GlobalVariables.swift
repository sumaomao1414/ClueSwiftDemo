...
func setToast(str:String) {
    ...
    let width = getLabWidth(labelStr: str, fontSize: 15, height: 20)
...
//MARK:获取字符串的宽度的封装
func getLabWidth(labelStr:String,fontSize:NSInteger,height:CGFloat) -> CGFloat {
    
    let statusLabelText: NSString = labelStr as NSString
    
    let size = CGSize(width: 900, height: height)
    
    let attributes = [NSFontAttributeName:UIFont.systemFont(ofSize: CGFloat(fontSize))] //设置字体大小
